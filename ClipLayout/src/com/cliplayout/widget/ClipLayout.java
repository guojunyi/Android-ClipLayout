...
public class ClipLayout extends RelativeLayout {
	...
	private Stack<ClipPage> mViewStack = new Stack<ClipPage>();
	...
	public void pushClipPage(final ClipPage clipPage) {
		...
		// clipPage attached
		if (clipPage.isAttached()) {
			return;
		}

		clipPage.setClipLayout(this);
		View view = clipPage.getView();
		...
		final int value = Math.max(clipPage.getCx(),
				getWidth() - clipPage.getCx())
				+ Math.max(clipPage.getCy(), getHeight() - clipPage.getCy());

		ValueAnimator animator = ValueAnimator.ofInt(clipPage.getStartRadius(),
				value);
	...
	public boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		if (mViewStack.size() > 1
				&& child == mViewStack.get(mViewStack.size() - 1).getView()) {
			return drawStackView(canvas, child, drawingTime);
		}

		if (mViewStack.size() > 1
				&& child == mViewStack.get(mViewStack.size() - 2).getView()) {
			return drawStackView(canvas, child, drawingTime);
		}
	...
	public boolean drawStackView(Canvas canvas, View child, long drawingTime) {
		...
		path.addCircle(clipPage.getCx(), clipPage.getCy(),
				clipPage.getRadius(), Path.Direction.CW);
